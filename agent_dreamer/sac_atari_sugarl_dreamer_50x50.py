...
if __name__ == "__main__":
    args = parse_args()
    env_list = ['alien', 'amidar', 'assault', 'asterix', 'bank_heist', 'battle_zone', 'boxing', 
                'breakout', 'chopper_command', 'crazy_climber', 'demon_attack', 'freeway', 
                'frostbite', 'gopher', 'hero', 'jamesbond', 'kangaroo', 'krull', 'kung_fu_master', 
                'ms_pacman', 'pong', 'private_eye', 'qbert', 'road_runner', 'seaquest', 'up_n_down']
    
    for env_name in env_list:
        args.env = env_name.lower()
        run_name = f"{args.env}__{os.path.basename(__file__)}__{args.seed}"
